...
@ Don't show skills for generic non-player units
mov   r0, #0x0
mov   r9, r0
mov   r0, r8
ldrb  r1, [r0, #0xB]
mov   r2, #0xC0
tst   r1, r2
beq   L1                          @ Check if unit is Enemy/NPC.

  ldr   r0, [r0]
  ldrb  r1, [r0, #0x4]
  cmp   r1, #0x3F
  ble   L1                        @ Enemy/NPC units above #0x3F are generic, unless boss flag is set.

    ldr   r1, [r0, #0x28]
    mov   r2, #0x80
    lsl   r2, #0x8
    tst   r1, r2
    bne   L1                      @ Check if unit is boss
...
ldr r0,=SS_ReservedText
lsl r0, #0x10
lsr r0, #0x10
draw_textID_at 21, 5, width=6, colour=Blue
