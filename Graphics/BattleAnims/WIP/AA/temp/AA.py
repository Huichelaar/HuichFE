...
def main():
  ...
  if len(sys.argv)==3:
    ...
    animname = classname + filename
    labelname = filename.replace(' ','_')
    outputname = animname + " Installer.event"
...
"""
    with open(outputname,'w') as o:
      o.write(outputText.format(animname=animname,classname=classname,filename=filename,labelname=labelname,framedata=process(filename),pal=b_to_hex(palettedata),rtl=b_to_hex(rtl),ltr=b_to_hex(ltr),graphicsdata=getSheets(sheetpaths),sectionData=b_to_hex(sectionData)))
    print("success")
  else:
